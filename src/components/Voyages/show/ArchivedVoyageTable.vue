    ...
    >
          ...
          <v-dialog v-model="dialogDelete" max-width="600px">
            <v-card>
              <v-card-title class="text-h5"
                >Are you sure you want to unarchive this item?</v-card-title
              >
              <v-card-actions>
                <v-spacer></v-spacer>
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn
          ...
          "
        >
          <v-icon color="#ffffff"> mdi-archive-arrow-up </v-icon>
        </v-btn>
        <v-btn
          depressed
          rounded
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      this.unarchiveVoyageAction({ id: this.editedItem.id })
        .then(() => {
    ...
    showAllMethod() {
      ...
      if (!this.showAll) {
        this.showAll = true;
        this.setAllArchivedVoyagesAction()
          .then(() => {
            this.initialize();
            this.setModuleShowToFalseAction();
          })
