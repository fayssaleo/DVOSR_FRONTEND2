...
export default {
  ...
  methods: {
    ...
    checkIfNumberFullAndFromAndToEmpty() {
      ...
      if (
        (!dgbohc_bfl_num_gbIsEmpty || !dgbohc_bfl_num_hcIsEmpty) &&
      ...
      if (
        ...
        this.warninText =
          "When 'FROM' datetime is inserted number of gear boxes and hatch covers must not be zero !";
        this.errorDialog = true;
        return true;
      }
      return false;
    ...
    checkIfNumberFullAndFromAndToEmpty2() {
      ...
      if (
        (dss_bfl_num_spIsEmpty || dss_bfl_fb_dnwIsEmpty || dss_bfl_fbIsEmpty) &&
        !dss_bfl_fromIsEmpty
      ) {
        this.warninText =
          "When 'FROM' datetime is inserted number of spreaders must not be zero and From Bay /Dish is NS or Wires must been selected !";
        this.errorDialog = true;
        return true;
      }
      return false;
